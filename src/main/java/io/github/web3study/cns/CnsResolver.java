...
import io.github.web3study.cns.contracts.generated.CNS;
import io.github.web3study.cns.contracts.generated.OffchainResolverContract;
import io.github.web3study.cns.contracts.generated.PublicResolver;
...
public class CnsResolver {
    ...
    private static final Logger log = LoggerFactory.getLogger(CnsResolver.class);
    ...
    public CnsResolver(Web3j web3j, long syncThreshold, int addressLength) {
        this.web3j = web3j;
    ...
    public CnsResolver(Web3j web3j, long syncThreshold) {
        this(web3j, syncThreshold, Keys.ADDRESS_LENGTH_IN_HEX);
    }

    public CnsResolver(Web3j web3j) {
     ...
     *
     * @param ensName our user input CNS name
     * @return PublicResolver
     * @deprecated <p>Use {@link CnsResolver#obtainOffchainResolver(String)} instead.
     */
    @Deprecated
    ...
    protected String resolveOffchain(
        ...
        if (EnsUtils.isEIP3668(lookupData)) {
            ...
            if (gatewayResult == null) {
                throw new CnsResolutionException("CCIP Read disabled or provided no URLs.");
            }
            ...
            if (EnsUtils.isEIP3668(resolvedNameHex)) {
                if (lookupCounter <= 0) {
                    throw new CnsResolutionException("Lookup calls is out of limit.");
                }
    ...
    protected String ccipReadFetch(List<String> urls, String sender, String data) {
        ...
        for (String url : urls) {
            ...
            } catch (JsonProcessingException | CnsResolutionException e) {
                ...
                if (response.isSuccessful()) {
                    ...
                    if (statusCode >= 400 && statusCode < 500) {
                        log.error(
                                ...
                                url,
                                response.message());
                        throw new CnsResolutionException(response.message());
                    }
    ...
    protected Request buildRequest(String url, String sender, String data)
        ...
        if (sender == null || !WalletUtils.isValidAddress(sender)) {
            throw new CnsResolutionException("Sender address is null or not valid");
        }
        if (data == null) {
            throw new CnsResolutionException("Data is null");
        }
        if (!url.contains("{sender}")) {
            throw new CnsResolutionException("Url is not valid, sender parameter is not exist");
        }
    ...
    private String getResolverAddress(String ensName) throws Exception {
        ...
        CNS ensRegistry =
                CNS.load(registryContract, web3j, transactionManager, new DefaultGasProvider());
